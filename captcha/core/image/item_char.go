...
func (item *ItemChar) drawHollowLine() *ItemChar {
	...
	lineColor := random.RandLightColor()
...
func (item *ItemChar) drawSineLine() *ItemChar {
	...
	//Y轴方向偏移量
	b := random.RandomFloatRange(float64(-item.height/4), float64(item.height/4))

	//X轴方向偏移量
	f := random.RandomFloatRange(float64(-item.height/4), float64(item.height/4))
	// 周期
	var t float64
	if item.height > item.width/2 {
		t = random.RandomFloatRange(float64(item.width/2), float64(item.height))
	} else if item.height == item.width/2 {
		t = float64(item.height)
	} else {
		t = random.RandomFloatRange(float64(item.height), float64(item.width/2))
	}
	w := (2 * math.Pi) / t

	// 曲线横坐标起始位置
	px1 := 0
	px2 := int(random.RandomFloatRange(float64(item.width)*0.8, float64(item.width)))

	c := random.RandDeepColor()
...
func (item *ItemChar) drawSlimLine(num int) *ItemChar {
	...
	for i := 0; i < num; i++ {
		...
		item.drawBeeline(point1, point2, random.RandDeepColor())
...
func (item *ItemChar) drawNoise(noiseText string, fonts []*truetype.Font) error {
	...
	for _, char := range noiseText {
		...
		c.SetFont(fonts2.RandFontFrom(fonts))
