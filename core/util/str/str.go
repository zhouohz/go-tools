...
// IsEmptyIfStr 判断对象是否为空串。
func IsEmptyIfStr(obj interface{}) bool {
	if obj == nil {
		return true
	}

	if str, isString := obj.(string); isString {
		return len(str) == 0
	}
...
// FillBefore 在字符串前填充字符，使其达到指定长度。
func FillBefore(str string, filledChar rune, length int) string {
	strLength := len(str)
	if strLength >= length {
		return str
	}

	fillCount := length - strLength
	fillStr := strings.Repeat(string(filledChar), fillCount)
	return fillStr + str
}

// FillAfter 在字符串后填充字符，使其达到指定长度。
func FillAfter(str string, filledChar rune, length int) string {
	strLength := len(str)
	if strLength >= length {
		return str
	}
