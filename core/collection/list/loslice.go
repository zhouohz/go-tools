...
func Fill[T tp.Cloneable[T]](collection []T, initial T) []T {
	result := make([]T, 0, len(collection))
...
func Repeat[T tp.Cloneable[T]](count int, initial T) []T {
...
func IsSorted[T tp.Ordered](collection []T) bool {
...
func IsSortedByKey[T any, K tp.Ordered](collection []T, iteratee func(item T) K) bool {
	size := len(collection)
...
// Play: https://go.dev/play/p/0r6VimXAi9H
func RangeFrom[T tp.Number](start T, elementNum int) []T {
	length := gfun.If(elementNum < 0, -elementNum).Else(elementNum)
...
// @param: end 结束值
// @param: step 步长
func RangeWithStep[T tp.Number](start, end, step T) []T {
	var result = make([]T, 0)
	if start == end || step == 0 {
		return result
	}
...
func Sum[T tp.Float | tp.Integer | tp.Complex](collection []T) T {
	var sum T = 0
...
func SumBy[T any, R tp.Float | tp.Integer | tp.Complex](collection []T, iteratee func(item T) R) R {
	var sum R = 0
	for _, item := range collection {
